...
#endif
		{
			if ( datasource == MegaCacheData.File )
			{
				if ( fs == null )
				{
					...
					{
						Debug.Log("ohhhhhhhhhhhh");
						fullpath = Application.absoluteURL;
						Debug.Log("before: " + fullpath);
						for (int i = fullpath.Length - 1; i >= 0; --i)
						{
							if (fullpath[i] == '/' || fullpath[i] == '\\')
							{
								fullpath = fullpath.Remove(i+1);
								break;
							}
						}
						Debug.Log("after: " + fullpath);
					}
					else
					{
					...
					fullpath += file;
					Debug.Log("ffffffffffffffff: " + fullpath);
